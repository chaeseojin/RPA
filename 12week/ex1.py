...
x_train, x_text, y_train, y_test = train_test_split(x_data, y_data, test_size=0.2)

from sklearn.neural_network import MLPRegressor
model_mlp = MLPRegressor().fit(x_train, y_train)
y_pred_mlp = model_mlp.predict(x_text)

df_x_test = pd.DataFrame(x_text, columns=['egg_weight', 'moavement', 'food'])
